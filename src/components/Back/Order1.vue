      ...
      <div class="content">
        ...
        <div class="order_text"
          ...
          <div>
            ...
            <p v-if="routerName == '新建云主机'" class="o-fm">当前黑龙江、内蒙古、山东三地暂时无法提供域名备案，您可以选择邻省进行备案</p>
            <p class="o-fm">请牢记您所设置的密码，如遗忘可登录云服务器控制台重置密码。<a class="blue_font" href="/support_docs/kiRWuMFJd_kmKQJcCNq.html" target="_blank" >查看</a></p>
            <p class="o-fm" v-if="isNotBuyDisk || routerName == '新建云硬盘'">云服务器购买成功后，数据盘默认是未挂载的情况，需要自行格式化硬盘后，挂载分区后才能在云服务器内看到。<a class="blue_font" href="support_docs/kieLofe4O_3BKrwYD0i.html" target="_blank">查看windows如何格式化、</a><a class="blue_font" href="/support_docs/kieLofe4O_l455ekGff.html" target="_blank">Linux如何格式化</a></p>
...
export default {
            ...
            for (var index in params.row["资源"]) {
              ...
              for (var key in params.row["资源"][index]) {
                if (key == "存储") {
                  ...
                  this.isNotBuyDisk =
                    dis.substring(dis.indexOf(",") + 1, dis.length) !=
                    "无数据盘"
                      ? true
                      : false;
                } else if (key == "配置(新)") {
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "span",
              {},
              params.row.订单状态 == 0 || params.row.订单状态 == undefined
                ? params.row.类型
                : "--"
            );
          }
        ...
        },
        {
          ...
          render(h, obj) {
            return h(
              "span",
              {},
              obj.row["优惠价"] != undefined ? obj.row["优惠价"] : "--"
            );
          }
        },
        {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    let orderS =
      sessionStorage.getItem("countOrder") == "undefined"
        ? null
        : sessionStorage.getItem("countOrder");
  ...
  methods: {
    ...
    setOrder(response) {
      if (response.status == 200 && response.data.status == 1) {
        ...
        if (response.data.result.data[0].configure != undefined) {
          if (
            JSON.parse(response.data.result.data[0].configure).constructor ==
            Array
          ) {
            sessionStorage.setItem(
              "orderZoneId",
              JSON.parse(response.data.result.data[0].configure)[0].zoneid
            );
          } else {
            sessionStorage.setItem(
              "orderZoneId",
              JSON.parse(response.data.result.data[0].configure).zoneid
            );
          }
        }
        let orderStatus = "";
        ...
        sessionStorage.setItem(
          "routername",
          response.data.result.data[0].goodstype
        );
        this.orderData = response.data.result.data.map(item => {
          ...
          if (item.discountmessage != undefined) {
            data.discountmessage = item.discountmessage.substring(
              item.discountmessage.indexOf("，") - 6,
              item.discountmessage.indexOf("，") - 3
            );
          } else {
        ...
        this.orderState = data["订单状态"]?data["订单状态"]:0;
          if (data["订单状态"]) {
            ...
            this.couponInfo.cost +=
              data["订单状态"] == 1 ? 0 : Number(item.cost.toFixed(2));
            this.couponInfo.totalCost +=
              data["订单状态"] == 1 ? 0 : Number(item.cost.toFixed(2));
        ...
        if (response.data.result.data[0].discountmessage) {
          this.showFree = JSON.parse(
            response.data.result.data[0].discountmessage
          );
        }
    ...
    exchange() {
      this.$http
        .get("user/receiveTicketForUser.do", {
          params: {
    ...
    getWalletsBalance() {
      this.$http
        .post("device/DescribeWalletsBalance.do", {})
        .then(res => {
        ...
        .catch(err => {
          if (err) this.$Message.info("网络异常，获取现金券失败请重试");
        });
    },
  ...
  computed: {
    otherSpentCost() {
      ...
      if (cost < 1117) {
        ...
        this.spentCostNode =
          "可领取3100元+ 1000元+ 350元 + 50元苏宁卡/京东E卡！";
        return 31117 - cost;
      } else {
        return 0;
        this.spentCostNode =
    ...
    deductionPrice() {
          ...
          this.couponInfo.couponList.forEach(item => {
            if (item.operatorid == this.couponInfo.selectTicket) {
              if (item.tickettype == 1) {
                money = (
                  this.couponInfo.cost -
                  this.couponInfo.cost * item.money
                ).toFixed(2);
      ...
      if (this.groupList[0] == "cash" || this.groupList[1] == "cash") {
        ...
        if (
          this.couponInfo.cost < this.couponInfo.cash ||
          this.couponInfo.cost == this.couponInfo.cash
        ) {
    ...
    vipPrice() {
      if (this.vipName != "" || this.vipName != undefined) {
        ...
        if (
          this.couponInfo.selectTicket != "" &&
          (this.groupList[0] == "cash" || this.groupList[1] == "cash")
        ) {
          this.couponInfo.couponList.forEach(item => {
            if (item.operatorid == this.couponInfo.selectTicket) {
              if (item.tickettype == 1) {
                if (this.couponInfo.cost > this.couponInfo.cash) {
                  money = (
                    this.couponInfo.cash -
                    (this.couponInfo.cost -
                      Number(this.couponInfo.cost * item.money))
                  ).toFixed(2);
                }
                if (
                  this.couponInfo.cost < this.couponInfo.cash ||
                ...
                if (
                  this.couponInfo.cost - item.money < this.couponInfo.cash ||
                  this.couponInfo.cost - item.money == this.couponInfo.cash
                ) {
      ...
      handler: function() {
        if (this.vipName == "" || this.vipName == undefined) {
          this.couponInfo.couponList.forEach(item => {
            if (item.operatorid == this.couponInfo.selectTicket) {
              if (item.tickettype == 1) {
                this.couponInfo.totalCost = Number(
                  (this.couponInfo.cost * item.money).toFixed(2)
                );
              } else if (item.tickettype == 0) {
                this.couponInfo.totalCost = Number(
                  (this.couponInfo.cost - item.money).toFixed(2)
            ...
            this.couponInfo.couponList.forEach(item => {
              if (item.operatorid == this.couponInfo.selectTicket) {
                if (item.tickettype == 1) {
                  if (
                    this.couponInfo.cash >
                      Number((this.couponInfo.cost - item.money).toFixed(2)) ||
                    this.couponInfo.cash ==
                      Number((this.couponInfo.cost - item.money).toFixed(2))
                  ) {
                    this.couponInfo.totalCost = Number(
                      this.couponInfo.cash - this.couponInfo.cost * item.money
                    ).toFixed(2);
                  }
                  if (
                    this.couponInfo.cash <
        ...
        if (
          ...
          if (
            ...
            if (
              ...
              if (
                this.couponInfo.cash > this.couponInfo.cost ||
                this.couponInfo.cash == this.couponInfo.cost
              ) {
                this.couponInfo.totalCost = 0;
              }
              ...
              if (
                ...
                this.couponInfo.totalCost = Number(
                  (this.couponInfo.cost - this.couponInfo.cash).toFixed(2)
                );
              }
            }
          }
  ...
  .wrapper {
    ...
    .content {
      ...
      .coupon {
        ...
        .coupon_radio::-webkit-scrollbar-thumb {
          /*滚动条里面小方块*/
          border-radius: 10px;
          -webkit-box-shadow: inset 0 0 5px rgba(0, 0, 0, 0.2);
          background: rgba(216, 216, 216, 0.5);
        }
        .coupon_radio::-webkit-scrollbar-track {
          /*滚动条里面轨道*/
          -webkit-box-shadow: inset 0 0 5px rgba(0, 0, 0, 0.2);
          border-radius: 10px;
          background: rgba(216, 216, 216, 0.5);
