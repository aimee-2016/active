        ...
        <Tabs type="card" :animated="false" style="margin-top:20px;">
          <TabPane label="启动配置">
            <div>
              ...
              <a href="/support_docs/wHfINiD0y_wHy1fIdoD.html" style="float: right;line-height: 30px;" target="_blank">查看帮助文档</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取启动配置
      selectAllElastic() {
        this.$http.get('elasticScaling/listElasticScalingRunConfig.do', {}).then(res => {
      ...
      //获取伸缩组列表
      selectAllTelescopic() {
        this.$http.get('elasticScaling/listExpansionGroups.do', {}).then(res => {
          if (res.status == 200 && res.data.status == 1) {
      ...
      getAllSelect() {
        var aa = true;
        this.$http.get('loadbalance/listLoadBalanceRoleAndInterLoadBalance.do', {}).then(res => {
          if (res.status == 200 && res.data.status == 1) {
