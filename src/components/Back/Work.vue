            ...
            <Tab-pane label="发起工单" name="发起工单">
              ...
              <div class="new-order">
                ...
                <div class="question">
                  ...
                  <span class="a">购买成功后可以通过控制台升级带宽，新睿云服务器最大支持100Mbps带宽。</span>
                  <span class="q">2、购买后是否支持升级？</span>
                  <span class="a">购买的默认云服务器无法升级配置，仅可升级带宽，定制云服务器可随时根据需要升级配置。</span>
                  ...
                  <span class="a">请根据购买之后您收到登录名和密码登陆云服务器，您可以在消息中心、绑定邮箱／手机短信中查看账户名和密码。如您忘记密码可通过控制台，先将云服务器关闭后，再进行重置密码。</span>
                  <span class="q">5、云服务器远程连接失败？</span>
                  <span class="a">系统默认打开防火墙22端口（linux系统远程连接）和3389端口（windows系统远程连接），如果关闭了，通过客户端进行云服务器远程连接需要打开相应端口。</span>
                  <span class="q">6、为何无法访问我的服务器上的服务？</span>
                  <span class="a">请先检查您的防火墙设置。为了加强云服务器的安全性， 初始状态下每个防火墙只默认打开22、3389、80、443端口和ping命令，如需要访问数据库、FTP等服务，请添加相应端口。</span>
                </div>
              </div>
            ...
            <Tab-pane label="处理中的工单" name="处理中的工单">
              <div class="operating">
                    ...
                    <div v-for="(item,index) in operatingOrder" :key="index" class="item" style="width:561px;background:rgba(255,255,255,1);box-shadow:0px 2px 8px 0px rgba(0,0,0,0.2);border-radius:10px;padding: 20px;margin: 0 0 30px 3px;">
                      ...
                      <div style="display: flex;flex-wrap: wrap">
                        ...
                        <span style="width:62%;margin-top: 10px;">持续时间 : {{item.timeago}}</span>
												<span v-if="item.pictureMap!=null" v-for="ited in item.pictureMap">
                ...
                <div class="question">
                  ...
                  <span class="q">6、为何无法访问我的服务器上的服务？</span>
                  <span
                    class="a">请先检查您的防火墙设置。为了加强云服务器的安全性， 初始状态下每个防火墙只默认打开22、3389、80、443端口和ping命令，如需要访问数据库、FTP等服务，请添加相应端口。</span>
                </div>
              </div>
            </Tab-pane>
                    ...
                    <div v-for="(item,index) in closingOrder" :key="item.id" class="item" style="background:rgba(255,255,255,1);box-shadow:0px 2px 8px 0px rgba(0,0,0,0.2);border-radius:10px;padding: 20px;width:561px;margin: 0 0 30px 3px;">
                      ...
                      <div style="display: flex; flex-wrap: wrap;">
                        ...
                        <span style="width:62%;margin-top: 10px;">经过时间 : {{item.timeago}}</span>
												<span v-if="item.pictureMap!=null" v-for="ited,index in item.pictureMap">
													<span style="width:100%;" ><img :src="ited" style="width: 80px;height: 80px;margin: 20px 20px 0 0;" /></span>
												</span>
                      </div>
                      <div class="operating-menu">
                        <span @click="viewDetail(item)">查看详情</span>
                  ...
                  <div class="item">
                    ...
                    <div class="reply-wrapper" ref="reply">
                      <div class="item" style="border-bottom:1px solid #d8d8d8">
											...
											<span v-if="orderDetail[2][0].pictureMap!=null" v-for="ited,index in orderDetail[2][0].pictureMap">
												<span style="width:100%;" ><img :src="ited" style="width: 80px;height: 80px;margin: 7px 20px 15px 0;" /></span>
											</span>
                      </div>
      ...
      reply() {
        ...
        this.$http.get(url, {
          ...
          if (response.status == 200 && response.data.status == 1) {
						this.testurl1= []
						this.uploadList1=[]
            this.orderDetail[3].push({g_reply: response.data.msg.g_reply, uname: null, repdate: new Date().getTime(), pic: response.data.msg.pic,pictureMap:response.data.msg.pictureMap})
            this.editorValue = ''
          }
					else 
					{
