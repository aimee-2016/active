      ...
      <div class="modal-body">
        <form>
          ...
          <div style="position:relative">
            ...
            <img :src="imgSrc" @click="imgSrc=`https://huodong.xrcloud.ne/user/getKaptchaImage.do?t=${new Date().getTime()}`">
          </div>
        </form>
      </div>
      ...
      this.$http.get('user/login.do', {
        ...
        if (response.status == 200 && response.statusText == 'OK') {
          if (response.data.status == 1) {
            ...
            this.imgSrc = `https://huodong.xrcloud.ne/user/getKaptchaImage.do?t=${new Date().getTime()}`
