    ...
    <Modal v-model="loginModal" width="420" class="login-modal" :scrollable="true">
      ...
      <div class="modal-body">
        <form>
          ...
          <div style="position:relative">
            ...
            <img :src="imgSrc" @click="imgSrc=`https://csactivity.xrcloud.net/user/getKaptchaImage.do?t=${new Date().getTime()}`">
          </div>
        </form>
      </div>
      <div slot="footer" class="modal-foot">
        ...
        <div>
          ...
          <a href="https://csi.xrcloud.net/register" target="_blank" class="btn-register">
            立即注册
            <span>注册完成之后记得回到活动页面领取50G存储包哦！</span>
          </a>
          <a href="https://csi.xrcloud.net/resetnew"  target="_blank" style="color:#0EB4FA;cursor:pointer;margin-top: 10px;float:right;font-size:14px">
            忘记密码
          </a>
        </div>
      </div>
      ...
      if ((!this.authInfo)|| (this.authInfo&&this.authInfo.authtype==0&&this.authInfo.checkstatus!=0)||(!this.authInfoPersion &&this.authInfo&&this.authInfo.authtype==1&&this.authInfo.checkstatus!=0)||(this.authInfoPersion&&this.authInfoPersion.checkstatus!=0 && this.authInfo&&this.authInfo.checkstatus!=0)) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            window.open('https://csi.xrcloud.net/usercenter', 'self')
          }
        })
        return
      }
