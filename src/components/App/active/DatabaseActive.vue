    ...
    <div class="banner">
      <div class="banner-content">
        <div class="banner-text">
          ...
          <p>热门云数据库产品免费试用60天</p>
          <p>
            <span>0元试用 押金闪退</span>每天不限量
          </p>
        </div>
        <div class="banner-img">
          <img src="../../../assets/img/active/dbActive/dba-banner2.png" alt="banner" />
        </div>
      </div>
    </div>
    <div class="center">
      <div class="da-product">
        ...
        <p>
          实例创建完成后请前往控制台使用，试用期结束可正常续订
          <span @click="roll(2600)">活动规则</span>
        </p>
        <div class="products" v-for="(item,index) in products">
          ...
          <div class="product-content">
            <ul class="product-config">
              <li>
                CPU
                <span>{{ item.config.gpu}}核</span>
              </li>
              <li>
                内存
                <span>{{ item.config.memory}}G</span>
              </li>
              <li style="width: 20%">
                系统盘
                <span>{{ item.config.systemDisk}}G</span>
              </li>
              <li style="width: 20%">
                数据盘
                <span>{{ item.config.wsx}}G</span>
              </li>
              <span>数据中心：</span>
              <Select
                v-model="item.config.zoneId"
                class="db-select"
                style="width:140px;"
                @on-change="getOriginalPrice(index)"
              >
                <Option
                  v-for="item in areaGroup"
                  :value="item.value"
                  :key="item.value"
                >{{ item.name }}</Option>
              </Select>
            ...
            <div class="product-price">
              <div class="price">
                <p>
                  押金：
                  <span>¥{{ item.config.currentPrice }}</span>
                </p>
                <p>
                  原价：
                  <span>¥{{ item.config.originalPrice}}</span>
                </p>
              </div>
      ...
      <div class="da-advantage">
        ...
        <div>
          <ul
            ...
            @mouseover="advSelect = index"
            @mouseout="advSelect = 5"
          >
            <li>
    ...
    <div style="background:linear-gradient(0deg,rgba(255,235,222,1),rgba(255,255,255,1));">
      <div class="da-rule">
        ...
        <dl>
          ...
          <dd>3、领取说明：本活动中所有产品均需充值对应的押金后才可使用，领取后宽限期（云服务器领取后到开始使用）为3天，保留期（云服务器到期或者用户主动删除后回收站保留时间）为7天；数据库产品未到期可提前申请删除，删除后或者数据库产品使用到期后押金转到余额账户，可用于购买和续费；或者可以向客服申请退还余额，七个工作日内即可原路退回。仅可选择四款云数据库产品中的一款产品进行免费试用，每个用户仅限参与一次。同一手机号对应的多个账号、同一实名认证用户等满足同一条件的均视为一个用户。</dd>
          <dd>4、试用产品使用规则：</dd>
    ...
    <Modal v-model="showModal.rechargeHint" :scrollable="true" :closable="false" :width="390">
      <div class="modal-content-s" style="padding: 30px 30px 0 50px">
        <div>
          ...
          <p class="lh24">
            本免费活动充值押金
            <span style="color: #D0021B ">{{ cashPledge }}</span>元，数据库到期或删除时押金自动退还到账户余额。
          </p>
        </div>
      </div>
    ...
    <Modal v-model="showModal.getSuccessModal" :scrollable="true" :closable="false" :width="390">
      <div class="modal-content-s" style="padding: 30px 30px 0 50px">
        <div>
          ...
          <strong>提示</strong>
          <p class="lh24">恭喜您押金已冻结完成，数据库领取成功，数据库在实名认证之前只可保留3天，请尽快使用。</p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
        <Button @click="showModal.getSuccessModal = false">取消</Button>
        <a href="https://csi.xrcloud.net/clouddatabase/">
    ...
    <Modal v-model="showModal.payDefeatedModal" width="640" :scrollable="true">
      ...
      <div class="universal-modal-content-flex">
        <div class="modal-p">
          ...
          <p>
            <img src="../../../assets/img/sceneInfo/si-defeated.png" alt="抱歉" />
  ...
  metaInfo: {
    ...
    meta: [
      ...
      },
      {
        ...
        name: "description",
        content:
          "新睿云推出云数据库产品免费试用60天活动，云数据库产品包含：Mysql 单实例、Redis分布式缓存服务、PostgreSQL 单实例、MongoDB 单实例；实例创建完成后请前往控制台使用，试用期结束可正常续订。"
      }
    ]
  },
  components: {
      ...
      advantages: [
        {
          ...
          title: "快速部署",
          desc:
            "选择规格后下单，几分钟内即可创建高可用的云数据库实例。具备完善的监控、告警功能，可立即投入使用，立即创造价值。"
        },
        {
        ...
        },
        {
          ...
          title: "弹性扩容",
          desc:
            "弹性扩容，按需升级，提高资源利用率，降低使用成本；有效满足业务发展对数据库性能的要求。"
        },
        {
          src: require("../../../assets/img/active/dbActive/dba-icon9.png"),
          title: "运维便捷",
          desc:
            "监控、自动告警功能有效降低用户的数据库运维和服务器网络运维成本，使数据库运维更加便捷、高效，减少人力和设备投入。"
        },
        {
          src: require("../../../assets/img/active/dbActive/dba-icon10.png"),
          title: "超高性能",
          desc:
            "全SSD本地磁盘存储，支持最大64G 内存、1T 以上磁盘的数据库实例，轻松应对高并发、大规模数据处理需求。"
        }
      ],
      advSelect: 5,
      scenarios: [
        {
          title: "新生互联网/移动APP",
          desc:
            "场景描述：云数据库可作为产品可靠的服务端数据库存储介质，快速便捷的拥有完整的数据库服务，无需购买昂贵的服务器、操作系统和数据库管理系统等软硬件，避免资源闲置浪费。"
        },
        {
          title: "高速发展的企业",
          desc:
            "场景描述：企业迅速发展过程中，业务量快速增长，需要高性能的数据库服务支持，同时为应对不断增加的业务量，可使用一键创建功能快速扩展数据库，大幅提高查询能力。"
        },
        {
          title: "传统企业及转型中企业",
          desc:
            "场景描述：被陈旧、繁琐、复杂的上一代数据库系统拖累，急于使用更好、更快、更稳定的新一代数据库服务，同时保留DBA灵活、高级的操作空间，助力企业转型。"
        }
      ],
      showModal: {
        rechargeHint: false,
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            let param = h(
              "li",
              {},
              "内存： " + params.row.configs.memory + "G"
            );
            let param1 = h(
              "li",
              {},
              "系统盘： " + params.row.configs.systemDisk + "G"
            );
  ...
  methods: {
    ...
    getOriginalPrice(index) {
      ...
      axios.get(url, { params: params }).then(res => {
        if (res.data.status == 1) {
          this.products[index].config.originalPrice =
            res.data.result.originalPrice;
        }
      });
    },
    roll(val) {
...
.banner {
  height: 400px;
  background: url("../../../assets/img/active/dbActive/dba-banner1.png") center
      no-repeat,
    linear-gradient(90deg, #ffebde, #fff);
  .banner-content {
    width: 1200px;
    height: 400px;
    margin: 0 auto;
  ...
  .da-product {
    ...
    .products {
      ...
      .product-title {
        height: 70px;
        padding: 20px;
        background: url("../../../assets/img/active/dbActive/dba-banner3.png")
          no-repeat;
        &.other {
          background: url("../../../assets/img/active/dbActive/dba-banner4.png")
            no-repeat;
        }
        > p {
          font-size: 24px;
  ...
  .da-scenario {
    ...
    .scenario {
      ...
      > p {
        ...
        text-align: center;
        &.one {
          background: url("../../../assets/img/active/dbActive/dba-banner5.png")
            no-repeat;
        }
        &.two {
          background: url("../../../assets/img/active/dbActive/dba-banner6.png")
            no-repeat;
        }
        &.three {
          background: url("../../../assets/img/active/dbActive/dba-banner9.png")
            no-repeat;
        }
      }
      p:nth-child(2) {
