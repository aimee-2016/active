  ...
  <div style="background: #FFF">
    ...
    <div style="background:linear-gradient(0deg,rgba(255,235,222,1),rgba(255,255,255,1));">
      <div class="da-rule">
        ...
        <dl>
          ...
          <dd>
            3、领取说明：本活动中所有产品均需充值对应的押金后才可使用，领取后宽限期（云服务器领取后到开始使用）为3天，保留期（云服务器到期或者用户主动删除后回收站保留时间）为7天；数据库产品未到期可提前申请删除，删除后或者数据库产品使用到期后押金转到余额账户，可用于购买和续费；或者可以向客服申请退还余额，七个工作日内即可原路退回。仅可选择四款云数据库产品中的一款产品进行免费试用，每个用户仅限参与一次。同一手机号对应的多个账号、同一实名认证用户等满足同一条件的均视为一个用户。
          </dd>
          <dd>4、试用产品使用规则：</dd>
    ...
    <Modal v-model="showModal.inConformityModal" :scrollable="true" :closable="false" :width="390">
      <div class="modal-content-s" style="padding: 30px 30px 0 50px">
        <div>
          ...
          <p class="lh24">您好，您不符合本活动的参与条件，去<span style="color: #2A99F2;cursor: pointer" @click="$router.push('/')">活动中心</span>看看其他活动吧！如果有其他需要可联系我们销售或者客服。
          </p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
    ...
    <Modal v-model="showModal.getSuccessModal" :scrollable="true" :closable="false" :width="390">
      ...
      <p slot="footer" class="modal-footer-s">
        ...
        <a href="https://csi.xrcloud.net/clouddatabase/"><Button type="primary">查看数据库</Button></a>
      </p>
    </Modal>
    <!-- 支付充值失败 -->
    ...
    <Modal v-model="showModal.authenticationSuccess" width="640" :scrollable="true">
      ...
      <div style="text-align:center;padding:40px 0;">
        <img src="../../../assets/img/payresult/paySuccess.png"
             style="width:36px;vertical-align:middle;margin-right:10px;" alt="支付成功">
    ...
    <Modal v-model="showModal.authenticationError" width="640" :scrollable="true">
      ...
      <div style="text-align:center;padding:40px 0;">
        <img src="../../../assets/img/payresult/payFail.png"
             style="width:36px;vertical-align:middle;margin-right:10px;" alt="支付失败">
