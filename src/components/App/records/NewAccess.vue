<template>
  <div style="background: FFF">
    ...
    <div class="body-bottom">
      <div class="content">
        <h2>备案区域</h2>
        <p class="recordsArea-but">
          <img src="../../../assets/img/records/records-icon7.png" alt="icon" />
          {{ area }}
        </p>
        <h3>备案信息验证</h3>
        <Form
          ref="filingInformation"
          :model="filingInformation"
          :rules="filingInformationRuleValidate"
          :label-width="120"
        >
          ...
          <FormItem label="请选择备案主体" prop="mainRecordId" v-if="filingInformation.mainRecord =='our'">
            <Select
              v-model="filingInformation.mainRecordId"
              style="width:157px;margin-right: 10px"
              placeholder="请选择备案主体"
              @on-change="changeMainRecordId"
            >
              <Option
                v-for="item in filingInformation.mainRecordIdList"
                :value="item.id"
                :key="item.id"
              >
                {{
          ...
          <FormItem label="主体单位所属区域" prop="district">
            <Select
              v-model="filingInformation.province"
              style="width:157px;margin-right: 10px"
              placeholder="请选择省"
              @on-change="changeProvince"
            >
              <Option
                v-for="item in filingInformation.provinceList"
                :value="item.name"
                :key="item.name"
              >
                {{
            ...
            <Select
              v-model="filingInformation.city"
              style="width:157px;margin-right: 10px"
              placeholder="请选择市"
              @on-change="changeCity"
            >
              <Option
                v-for="item in filingInformation.cityList"
                :value="item.name"
                :key="item.name"
              >
                {{ item.name
            ...
            <Select
              v-model="filingInformation.district"
              style="width:157px"
              placeholder="请选择区"
              @on-change="changeDistrict"
            >
              <Option v-for="item in filingInformation.districtList" :value="item" :key="item">
                {{ item
                }}
              </Option>
            </Select>
          ...
          <FormItem label="主体备案号" prop="websiteRecordNumber">
            <Input
              v-model="filingInformation.websiteRecordNumber"
              placeholder="请输入网站备案号"
              style="width: 500px"
            ></Input>
          </FormItem>
          <FormItem label>
            <p class="formP" style="margin: 0">
              您所填写备案号不能存在
              <Tooltip content="所备案网站域名已被注销或域名持有者不再是备案主体" placement="top">
                <span>空壳主体</span>
              </Tooltip>，否则会导致审核失败
            </p>
          </FormItem>
          <FormItem label="ICP备案密码" prop="IPCPassword">
            <Input
              v-model="filingInformation.IPCPassword"
              placeholder="请输入ICP备案密码"
            ...
            <img
              ...
              src="../../../assets/img/reset/closeEye.png"
              class="eyeIcon"
              style="position: absolute;top: 26%;left: 43%;"
              alt="close"
            />
            <img
              v-show="inputType=='text'"
              @click="inputType='password'"
              src="../../../assets/img/reset/eye.png"
              class="eyeIcon"
              style="position: absolute;top: 26%;left: 43%;"
              alt="eye"
            />
    ...
    <Modal v-model="showModal.hint_1" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p class="lh24">您的账号下未检测到主体，请选择“已在其他接入商进行备案”</p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
        ...
        <Button
          type="primary"
          @click="filingInformation.mainRecord = 'other',showModal.hint_1 = false"
        >确认</Button>
      </p>
    </Modal>
    ...
    <Modal v-model="showModal.hint_2" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p class="lh24">您的账号下面已有主体，请选择“已在新睿云提交备案”</p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
        ...
        <Button
          type="primary"
          @click="filingInformation.mainRecord = 'our',showModal.hint_2 = false"
        >确认</Button>
      </p>
    </Modal>
  </div>
...
export default {
  ...
  data() {
    ...
    const validateArea = (rule, value, callback) => {
      if (
        this.filingInformation.province == "" ||
    ...
    return {
      ...
      filingInformationRuleValidate: {
        mainRecordId: [
          { required: true, message: "请选择备案主体", trigger: "change" }
        ],
        district: [
  ...
  methods: {
    ...
    // 获取主体列表
    getMainRecordIDList() {
      let url = "recode/listMainCompany.do";
      this.$http.get(url).then(res => {
        if (res.data.status == 1 && res.status == 200) {
          this.filingInformation.mainRecordIdList = res.data.result;
        }
      });
    },
    // 获取备案信息
    ...
    changeMainRecordId(val) {
      this.filingInformation.mainRecordId = val + "";
    },
    // 重新选择省份
    changeProvince(val) {
      this.filingInformation.city = "";
      ...
      this.$refs.filingInformation.validate(valid => {
        if (valid) {
          ...
          this.$http
            .get(url, {
              params: {
                id: this.filingInformation.mainRecordId
              }
            })
            .then(res => {
              if (res.data.status == 1 && res.status == 200) {
                let mainUnitInformation = res.data.result[0];

                this.mainUnitInformation.maincompanyarea =
                  mainUnitInformation.maincompanyarea;
                this.mainUnitInformation.certificateType =
                  mainUnitInformation.maincompanycertificatestype;
                this.mainUnitInformation.unitProperties =
                  mainUnitInformation.maincompanynature;
                this.mainUnitInformation.certificateNumber =
                  mainUnitInformation.maincompanynumber;
                this.mainUnitInformation.unitName =
                  mainUnitInformation.maincompanyname;
                this.mainUnitInformation.certificatesResidence =
                  mainUnitInformation.maincompanycertificatesloaction;
                this.mainUnitInformation.mailingAddress =
                  mainUnitInformation.maincompanycommunicatlocation;
                this.mainUnitInformation.investorName =
                  mainUnitInformation.investorname;
                this.mainUnitInformation.legalPersonName =
                  mainUnitInformation.legalname;
                this.mainUnitInformation.legalPersonCertificateType =
                  mainUnitInformation.legalcertificatestype;
                this.mainUnitInformation.legalPersonIDNumber =
                  mainUnitInformation.legalcertificatesnumber;
                this.mainUnitInformation.officePhone =
                  mainUnitInformation.officenumber;
                this.mainUnitInformation.phoneNumber =
                  mainUnitInformation.companyphone;
                this.mainUnitInformation.emailAddress =
                  mainUnitInformation.companyemail;
                this.mainUnitInformation.province = this.filingInformation.province
                 this.mainUnitInformation.city = this.filingInformation.city
                  this.mainUnitInformation.district = this.filingInformation.district
                let accessInfo = {
                  mainBelongArea:
                    this.filingInformation.province +
                    "-" +
                ...
                let mainUnitInformationStr = JSON.stringify(
                  this.mainUnitInformation
                );
                sessionStorage.setItem(
                  "mainUnitInformationStr",
                  mainUnitInformationStr
                );
                sessionStorage.setItem(
                  "accessInfo",
                ...
                sessionStorage.setItem(
                  "mainRecordId",
                  this.filingInformation.mainRecordId
                );
                this.$router.push("/newRecordStepTwo");
              } else {
                this.$message.info({
