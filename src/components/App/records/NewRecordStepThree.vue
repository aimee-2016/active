  ...
  <div style="background: #FFF">
    ...
    <Modal v-model="showModal.qrCode" width="550" :scrollable="true" :mask-closable="false" :closable="false">
      ...
      <div class="universal-modal-content-flex qrcode-modal">
         ...
         <p v-show="!authStatus">请<span>{{ siteListStr[0]?siteListStr[0].basicInformation.principalName : '' }}</span>使用手机（推荐使用微信）扫描二维码，并根据提示完成人脸识别认证</p>
    ...
    methods: {
      ...
      setData(area, zoneId, recordsType, mainUnitInformationStr, siteListStr) {
        ...
        if (sessionStorage.getItem('siteParamsStr')) {
          ...
          this.uploadForm.combine = JSON.parse(sessionStorage.getItem('mainParamsStr')).hostCompanyUrl
          // 初始化其他资料
