<template>
  <div style="background: #FFF">
    ...
    <o-step
      ...
      :recordsTypeDesc="recordsTypeDesc"
      v-if="recordsType !=='新增备案'"
    ></o-step>
      ...
      <div class="selectWay">
        ...
        <div v-if="photograph === 1">
          <div class="description">
            <p>
              1.下载填写完成
              <a :href="checkListAddress">《网站备案信息真实性核验单》</a>，按要求填写完成并盖章。
            </p>
          ...
          <div class="footer">
            ...
            <p
              style="border-bottom: 1px solid #999999;width: 765px;padding-bottom: 20px;"
            >当天拍照完成，当天提交管局，速度较快。</p>
          </div>
          ...
          <div class="description">
            ...
            >
              <FormItem label="地址" prop="address">
                ...
                ></Input>
                <span
                  style="cursor: pointer;color: #377dff;margin-left: 10px;font-size: 14px"
                  @click="updateReceiveForm"
                  v-if="!curtainStatus"
                >使用新地址</span>
              </FormItem>
              <FormItem label="收件人" prop="person">
                <Input
                  v-model="receiveForm.person"
                  placeholder="请输入收件人"
                  style="width: 500px"
                  :readonly="canUpdate"
                ></Input>
    ...
    <div class="footer" v-show="photograph === 2 && nextStep  && curtainStatus ">
      ...
      <div class="upload">
            ...
            <div class="item-content">
              <div style="width:100%;">
                <Upload
                  ...
                  :on-success="photoImg"
                >
                  <div class="item-content-text" v-if="upload.photo===''">点击选择文件</div>
        ...
        <div class="uploadTitle" style="margin: 34px 0 10px 20px;">
          <div class="item" style="text-align: center">
            <img
              @click="imageViewShow=true"
              style="height: 203px;cursor: zoom-in"
              src="../../../assets/img/records/records-img6.png"
              alt="头像"
            />
          </div>
        </div>
      ...
      <div class="rule">
        ...
        <p>
          为保障您的资料顺利通过管局审核，请根据以上提示上传符合管局要求图片，如有问题可咨询客服专员
          <span
            style="color: red"
          >（建议您上传照片之前联系备案专员确认照片信息符合要求）</span>
          <a
            target="_blank"
            :href="`tencent://message/?uin=1746856113&amp;Site=www.cloudsoar.com&amp;Menu=yes`"
          >（QQ客服-备案）</a>
        </p>
      </div>
    ...
    <div
      class="ImageView is-active"
      style="padding-bottom: 10px;"
      v-show="imageViewShow"
      @click="imageViewShow=false"
    >
    ...
    <Modal v-model="showModal.logistics" width="550" :scrollable="true">
      ...
      <div class="logistics">
        ...
        <p>
          物流单号：
          <span style="color: #377dff">xxx</span>
        </p>
      </div>
      <div slot="footer" class="modal-footer-border">
        <Button type="primary" @click="showModal.logistics = false">确认</Button>
      </div>
    </Modal>
    <!-- 申请幕布提示 -->
    <Modal v-model="showModal.applyHint" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p
            class="lh24"
          >由于背景幕布需回收并多次使用，寄送幕布需要从您的账户冻结50元押金，待我们确认回收幕布之后，押金即解冻。注意：冻结押金不可用于购买和续费。您可以在费用中心查看冻结押金详情。（提示：若您寄回幕布的邮费超出50元，我们将从押金中扣除超出费用，若邮费超过100元，我们将拒收快递，由此造成的损失由您自行负责）</p>
        </div>
      </div>
    ...
    <Modal v-model="showModal.shortageHint" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p class="lh24">您的账户余额小于50元，冻结资金不足。请先充值。</p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
  ...
  methods: {
    ...
    getPersonInfo(response) {
      if (response.data.status === 1) {
        ...
        // 判断是否申请幕布
        if (
          response.data.result[0].mark3 != "" &&
          typeof response.data.result[0].mark3 != "undefined"
        ) {
          ...
          this.receiveForm.address =
            response.data.result[0].maincompanycommunicatlocation;
    ...
    applyCurtain(val) {
      ...
      let params = {
        ...
        mainCompanyCertificatesType: this.mainParams
        ...
        mainCompanyCertificatesLoaction: this.mainParams
          .mainCompanyCertificatesLoaction,
        mainCompanyCommunicatLocation: this.mainParams
      ...
      let list_web_message = JSON.parse(this.siteParams.list_web_message);
      if (sessionStorage.getItem("mainRecordId")) {
        list_web_message.forEach(item => {
          item.mainCompanyId = sessionStorage.getItem("mainRecordId");
        });
        this.siteParams.list_web_message = JSON.stringify(list_web_message);
        ...
        axios.post("recode/addMainCompany.do", params).then(response => {
          if (response.data.status == 1 && response.status == 200) {
            list_web_message.forEach(item => {
              item.mainCompanyId = response.data.result;
            });
            this.siteParams.list_web_message = JSON.stringify(list_web_message);
            axios.post("recode/addMainWeb.do", this.siteParams).then(res => {
              if (res.data.status == 1) {
    ...
    getCheckList() {
      let province = this.mainParams.mainCompanyArea
        ? this.mainParams.mainCompanyArea.split("-")[0]
        : this.recordInfo.maincompanyarea.split("-")[0];
      let mainUnit =
        this.mainParams.mainCompanyNature || this.recordInfo.maincompanynature;
      if (province == "浙江省") {
