  ...
  <div style="background: #FFF">
    ...
    <Modal v-model="showModal.recordInfo" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p class="lh24">
            您选择的区域未查询到符合要求的公网IP与云服务器，请先购买该资源然后在进行备案。
            <span style="color: #377dff;cursor: pointer;" @click="$router.push('/buy')">立即购买</span>
          </p>
        </div>
      </div>
    ...
    <Modal v-model="showModal.hint" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p class="lh24">您已在新睿云备过案，请选择新增网站进行备案</p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
    ...
    <Modal v-model="showModal.hasMainWep" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p class="lh24">您未在新睿云备过案，请选择新增备案进行下一步操作</p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
    ...
    <Modal v-model="showModal.hasRecord" :scrollable="true" :closable="false" :width="390">
      ...
      <div class="modal-content-s">
        <div>
          <p class="lh24">您当前的备案尚未完成，请完成当前备案完成后再进行新的备案</p>
        </div>
      </div>
      <p slot="footer" class="modal-footer-s">
...
export default {
  ...
  created() {
    this.$store.state.zoneList.forEach(item => {
      if (item.gpuserver === 2 || item.gpuserver === 0) {
        let params = {
          text: item.zonename,
          zoneId: item.zoneid,
