...
<script type="text/ecmascript-6">
  ...
  export default {
    metaInfo: {
      title: 'java、php、python集成开发环境 - 应用场景 - 新睿云', // set a title
      meta: [{                 // set meta
        name: 'keywords',
        content: '集成开发环境,java集成开发环境,php集成开发环境,python集成开发环境'
      },
        {                 // set meta
          name: 'description',
    ...
    methods: {
      ...
      nextStep() {
        ...
        axios.get('activity/jdugeTeam.do', {
          params: {sign: 'freeReceive',vmConfigId: this.currentSceneGroup.configGroup[this.index1].id}
        }).then(response => {
          if (response.status == 200 && response.data.status == 1) {
