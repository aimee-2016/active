<template>
  <div>
    <h1 id="hide-h1">云桌面</h1>
    <div class="scene-page" v-for="(item,currentIndex) in currentSceneGroup" v-if="item.currentScene == scene" :key="currentIndex"
      ...
      <div class="center">
        ...
        <div class="body">
          ...
          <div class="configGroup">
            <div v-for="(cfg,index1) in item.configGroup" :key="index1" class="config" :class="{gpu: scene == '游戏服务'||scene == '图形设计'|| scene == '人工智能'|| scene == '超级运算',hidden: index1 == 1 || index1 == 2}">
...
<script type="text/ecmascript-6">
  ...
  export default {
    metaInfo: {
      title: '免费云桌面 - 云桌面下载 - 应用场景 - 新睿云', // set a title
      meta: [{                 // set meta
        name: 'keywords',
        content: '云桌面,免费云桌面,云桌面下载'
      },
        {                 // set meta
          name: 'description',
          content: '云桌面是由新睿云所提供的云上虚拟Windows桌面服务，为用户提供随时随地高效接入PC的便利。云桌面可按需申请轻松使用，助您打造更精简、更安全、更低维护成本、更高服务效率的个人PC使用系统。'
        }]
    },
    components: {
      VueQArt
    ...
    data() {
      ...
      return {
        ...
        currentSceneGroup: [
          {
            currentScene: '云桌面',
            disc: ' 云桌面是由新睿云所提供的云上虚拟Windows桌面服务，为用户提供随时随地高效接入PC的便利。云桌面可按需申请轻松使用，助您打造更精简、更安全、更低维护成本、更高服务效率的个人PC使用系统。借助新睿云，无论您使用何种终端设备，云桌面都可以让您拥有完整的PC使用体验。云桌面为您提供持续、安全、稳定、高性价比的BYOD模式云端计算服务。',
            configGroup: [
              {
                title: '一个月',
                configs: [
    ...
    created() {
      this.setData('yundiannao')
      this.getRegion('yundiannao')
    },
    methods: {
      init() {
            ...
            if (response.data.status == 1 && response.status == 200) {
              ...
              axios.get(url, {
                ...
                if (res.status == 200 && res.data.status == 1) {
                  ...
                  this.toggleZone(this.currentSceneGroup[this.index1].configGroup[this.index2].zoneId)
                } else {
                  this.$message.info({
                    content: res.data.message
                  })
