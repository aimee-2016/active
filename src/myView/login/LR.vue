  ...
  <div class="mask">
    <form onsubmit="return false" v-if="T!='protocol'">
      ...
      <div class="wrapper" v-show="T=='login'">
        ...
        <div class="input-group">
          <div class="error-box" v-show="warning!=''">
            <img src="./img/LR-warning.png">
            <p>{{warning}}</p>
          </div>
          <div class="input-box">
            <img src="./img/LR-phone.png">
            <input v-model="loginForm.loginName" type="text" placeHolder="登录邮箱/手机号">
          </div>
          <div class="input-box">
        ...
        <div class="prompt-box">
          <span @click="T='register'">立即注册</span>
      ...
      <div class="register-wrapper" v-show="T=='register'">
        ...
        <i></i>
        <div class="input-group">
          <div class="error-box" v-show="registerWarning!=''">
            <img src="./img/LR-warning.png">
            <p>{{registerWarning}}</p>
          </div>
    ...
    <div v-if="T=='protocol'" class="protocol-wrapper">
      <div class="rulesContent">
        ...
        <p style="margin-top: 30px"><strong>本网站（www.xinruiyun.cn）所提供服务是由北京允睿讯通科技有限公司新睿云向本网站注册用户提供的云计算产品和相关服务，
          详见本网站相关产品及服务介绍。本协议由您和北京允睿讯通科技有限公司签订。</strong></p>
        <p style="margin-top: 20px; text-indent:0em"><strong>第一条 总则</strong></p>
        <p>1.1 北京允睿讯通科技有限公司（以下称“乙方”,网址：www.xinruiyun.cn）按照本协议的规定及不定期发布的操作规则向用户（以下称“甲方”）提供系列云产品及
          云业务服务（以下简称“ 本服务”）。</p>
        <p>1.2 甲方须同意: 乙方变更本服务或协议内容的，将在乙方本服务网络平台（www.xinruiyun.cn）（以下称“服务门户”）上以公告的方式予以公布，无需另行单独通知
        ...
        <p>2.1本服务主要包括乙方基于云计算资源、云存储资源、智能云网络、互联网接入等所提供的云服务器、云存储、等服务内容。协议项下本服务的具体内容，以服务门户上展示
        ...
        <p>4.2 甲方使用本服务开展互联网信息服务等相关业务的，甲方保证于本协议签订时及本协议有效期内持续具备开展该等业务所需的全部资质及/或履行相关手续。甲方应根据乙
          方要求签订《互联网云服务器用户入网责任书》（附件一）、《网络信息安全承诺书》（附件二）、《新睿云云服务器服务等级协议》（附件三）、并严格遵守在该等文件中的承诺。</p>
        ...
        <p style="text-indent:3em">（4）进行钓鱼、黑客、网络诈骗等非法行为，网站或空间中含有或涉嫌散播病毒、木马、恶意代码等，或通过虚拟云服务器对其他网站、云服务器进行涉嫌攻击
        ...
        <p style="margin-top: 20px; text-indent:0em"><strong>附件三  《新睿云云服务器服务等级协议》</strong></p>
        <p style="text-indent:0em"><strong>新睿云云服务器服务等级协议</strong></p>
        ...
        <p>2  乙方保证甲方云服务器系统99.95%（百分之九十九点九）的可用性。乙方保证交付给甲方的云服务器、操作系统、网络和公网IP地址可用。甲方收到乙方提供的操作系统用户名和密码后
          ，及时修改妥善保管，进行验证后反馈乙方（2个工作日未反馈乙方，默认为甲方已验证能正常使用），此后甲方对操作系统以及系统上安装的应用程序负责。以下原因导致的甲方云服务器不可用的情况例外：</p>
        ...
        <p>2 甲方需要乙方工程师直接对其云服务器进行操作时，需要向乙方工程师书面授权；甲方需在与乙方签订的协议中指定授权人（维护人），只有授权人有权利要求乙
  ...
  export default{
    ...
    methods: {
      ...
      sendRegisterCode(){
        ...
        if (this.registerForm.vailCode.trim() == '') {
          this.registerWarning = '请输入图片验证码'
          return
        }
        axios.get('user/code.do', {
          params: {
            aim: this.Q == 'phone' ? this.registerForm.phoneLogin : this.registerForm.emailLogin,
            type: '0',
            isemail: this.Q == 'phone' ? '0' : '1',
            vailCode: this.registerForm.vailCode,
          }
        }).then(response => {
          if (response.status == 200 && response.data.status == 1) {
            let countDown = 60
      ...
      register(){
        ...
        if (this.registerForm.password != this.registerForm.passwordConfirm) {
          this.registerWarning = '两次输入的密码不一致'
          return
        }
        if (!this.single) {
    ...
    computed: {
      enable(){
        return this.loginForm.loginName.trim() != '' && this.loginForm.password.trim() != '' && this.loginForm.vailCode.trim() != ''
      }
    }
  }
</script>
